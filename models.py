...
from keras.layers import Dense, SimpleRNN, LSTM, LSTM, Bidirectional, TimeDistributed, Conv1D, ZeroPadding1D
...
def blstm(units, input_dim=26, output_dim=29, dropout=0.2, numb_of_dense=3, cudnn=False, n_layers=1):
    ...
    if cudnn:
        for i in range(0, n_layers):
            x = Bidirectional(LSTM(units, kernel_initializer=kernel_init_rnn, bias_initializer=bias_init_rnn,
...
def deep_lstm(units, input_dim=26, output_dim=29, dropout=0.2, numb_of_dense=3, cudnn=False, n_layers=3):
    ...
    if cudnn:
        for i in range(0, n_layers):
            x = LSTM(units, kernel_initializer=kernel_init_rnn, bias_initializer=bias_init_rnn,
...
def ctc_lambda_func_sh(args):
    Alignment,y_pred, labels, input_length, label_length = args
    # the 2 is critical here since the first couple outputs of the RNN
    # tend to be garbage:
    # print "y_pred_shape: ", y_pred.shape
    y_pred = y_pred[:, 2:, :]
    # print "y_pred_shape: ", y_pred.shape
    return K.ctc_batch_cost_sh(Alignment,labels, y_pred, input_length, label_length)
